...
const handleView = (clientId) => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'game_machines') {
      ...
      result = result.sort((a, b) => {
        const aCommission = parseFloat(a.commission_deal);
        const bCommission = parseFloat(b.commission_deal);
  ...
  const handleMessage = (id) => {
    console.log(`Message about item with ID: ${id}`);
              ...
              <tr key={machine.machine_id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-2">
                    <button 
                      className="text-blue-600 hover:text-blue-900"
                      onClick={() => handleView(machine.machine_id)}
                    >
                      <EyeIcon className="h-5 w-5" />
                    </button>
                    <button 
                      className="text-green-600 hover:text-green-900"
                      onClick={() => handleMessage(machine.machine_id)}
                    >
                      <ChatBubbleLeftRightIcon className="h-5 w-5" />
                    </button>
                  </div>
              ...
              <tr key={machine.trtMachine_id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-2">
                    <button 
                      className="text-blue-600 hover:text-blue-900"
                      onClick={() => handleView(machine.trtMachine_id)}
                    >
                      <EyeIcon className="h-5 w-5" />
                    </button>
                    <button 
                      className="text-green-600 hover:text-green-900"
                      onClick={() => handleMessage(machine.trtMachine_id)}
                    >
                      <ChatBubbleLeftRightIcon className="h-5 w-5" />
                    </button>
                  </div>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-start md:items-center">
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md ${activeTab === 'commission' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Commission
          </button>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {activeTab === 'all' && (
          <>
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-sm font-medium text-gray-500">Total Game Machines</h3>
              <p className="text-2xl font-semibold text-gray-900">
                {filteredData.reduce((sum, client) => sum + client.no_of_game_machines, 0)}
              </p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-sm font-medium text-gray-500">Total TRT Machines</h3>
              <p className="text-2xl font-semibold text-gray-900">
                {filteredData.reduce((sum, client) => sum + client.no_of_trts, 0)}
              </p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {filteredData.length > 0 
                  ? (filteredData.reduce((sum, client) => {
                      const commission = parseFloat(client.commission_deal);
