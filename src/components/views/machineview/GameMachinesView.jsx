...
const GameMachinesView = () => {
  ...
  const allRegions = ['All', ...new Set(gameMachines.map(machine => machine.region))];

  // Filter machines based on selected region
  const filteredMachines = selectedRegion === 'All' 
    ? gameMachines 
    : gameMachines.filter(machine => machine.region === selectedRegion);
...
export default GameMachinesView;